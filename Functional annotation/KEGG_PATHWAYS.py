...
def main():
    ...
    ******************************************************
    *                                                    *
    *                Welcome to KEGG_PATHWAYS.py         *
    *                                                    *
    ******************************************************
    """

    script_name = "KEGG_PATHWAYS.py"
    creator = "Gabriel Dall'Alba"
    date = "2024-02-27"
    description = "This script imports kegg Pathway IDs from an Excel file and retrieves their names using the API from kegg."
    usage = "python KEGG_PATHWAYS.py <file>"
    ...
    for row in sheet.iter_rows():
        for cell in row:
            if cell.value and cell.value.startswith("ko") or cell.value and cell.value.startswith("map"):
                kegg_term = cell.value
    ...
    for kegg_term, locations in kegg_term_locations.items():
        ...
        if kegg_term in kegg_terms_dict:
            ...
            if not isinstance(result, int):
                ...
                kegg_line_one = kegg_term_name[0]
                kegg_line_one = kegg_line_one.split()

                kegg_line_two = kegg_term_name[1]
                kegg_line_two = kegg_line_two.split()
                
                #merge elements 1 to the end of the list
                kegg_line_two = " ".join(kegg_line_two[1:])

                print(f"{kegg_line_two} ({kegg_line_one[1]})")


                #merge the elements to be format name (symbol) (entry)
                kegg_term_name = f"{kegg_line_two} ({kegg_line_one[1]})"
